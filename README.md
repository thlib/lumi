...
It solves one problem: application code should be able to describe what the
DOM should show without manually finding, creating, moving, and updating DOM
nodes.
...
These are sources of lessons, not APIs Lumi must imitate. A dependency belongs
only when it fits Lumi's single component model and does not introduce another
language or ownership system.
