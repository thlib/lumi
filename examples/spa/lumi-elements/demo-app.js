...
/**
 * Registers a custom element's template, behavior, and presentation function.
 *
 * @param {string} name
 * @param {DefinitionOptions | (() => DefinitionOptions)} declaration
 */
export function defineElement(name, declaration) {
  if (definitions.has(name)) {
    throw new Error(`Component "${name}" is already defined`)
  }
  if (!name.startsWith('lu-')) {
    throw new Error(`Lumi custom element "${name}" must use the lu- prefix`)
  }
...
const shellPlan = [
  {at: ':scope', use: 'lu-app-shell'},
  {at: '#header', use: 'lu-header'},
  {at: '#sidebar', use: 'lu-navigation'},
]

/** @type {Readonly<Record<Route, ReadonlyArray<PlanEntry>>>} */
const routePlans = {
  overview: [
    {at: ':scope', use: 'lu-overview'},
    {
      at: '.focus > .content',
      use: 'lu-project-list',
    ...
    },
    {
      ...
      use: 'lu-activity-list',
      select: data => data.activities.slice(0, 3),
    },
  ],
  projects: [
    {at: ':scope', use: 'lu-projects'},
    {
      at: '.content',
      use: 'lu-project-list',
      select: data => {
  ...
  records: [
    {at: ':scope', use: 'lu-records'},
  ],
  activity: [
    {at: ':scope', use: 'lu-activity-page'},
    {
      at: '.activity-panel > .content',
      use: 'lu-activity-list',
 ...
 */
function documentTitle(route, memberId) {
